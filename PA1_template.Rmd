...
par(mfrow=c(1,2))
## dataset 1
boxplot(data.s1$totSteps, xlab = paste("Total steps a day (mean:", m1, ")"), main = "Excluding NAs")
abline(h=mean(data.s1$totSteps),col="red", lwd=2)
## dataset 2
boxplot(data.s2$totSteps, xlab = paste("Total steps a day (mean:",m2,")"), main = "Including NAs")
...
maxStepInterval <- sm1[sm1["x"]==max(sm1["x"]),1]
plot(x=sm1$Group, y=sm1$x, type="l", xlab=paste("interval(max steps at:", maxStepInterval , ")"), main="Excluding NAs", ylab="steps in 5 minues")
abline(v=sm1[sm1["x"]==max(sm1["x"]),1], col="green", lwd=2)
## dataset 2
par(mfrow=c(1,1))
sm1 <-aggregate(data.f2$steps, by=list(data.f2$interval), mean)
maxStepInterval <- sm1[sm1["x"]==max(sm1["x"]),1]
plot(x=sm1$Group, y=sm1$x, type="l", xlab=paste("interval(max steps at:", maxStepInterval, ")"), main="Including NAs", ylab="steps in 5 minues")
abline(v=sm1[sm1["x"]==max(sm1["x"]),1], col="green", lwd=2)

```

It is seen in the time series plot that the patterns have not changed due to inclusion and exclusion of the `NA's` in the dataset. The maximum steps interval remain same for both at 8:35 AM which is shown as green line in both plots.
...
library(lattice)
xyplot(data.s5$steps~data.s5$interval | data.s5$dateType, type="l", xlab = "Interval", ylab = "Number of steps",  layout=c(1,2))
```
